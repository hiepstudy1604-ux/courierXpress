...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (!error.response) {
        error.message =
          "Unable to connect to the API server. Please check that the backend is running (php artisan serve) and verify the API URL.";
      }
    }
...
export const CourierService = {
  ...
  getSuitableVehicles: (orderId: string) =>
    api.get(`/courier/${orderId}/suitable-vehicles`),
  assignVehicle: async (orderId: string, vehicleId: number) => {
    const res = await api.post(`/courier/${orderId}/assign-vehicle`, {
  ...
  findSuitableBranch: (orderId: string) =>
    api.post(`/courier/${orderId}/find-branch`),
};
...
export const AuthService = {
  ...
  checkEmail: (email: string) => api.post("/auth/email/check", { email }),
  requestEmailOtp: (email: string) =>
    api.post("/auth/email-otp/request", { email }),
  verifyEmailOtp: (email: string, code: string) =>
    api.post("/auth/email-otp/verify", { email, code }),
};
...
export const ReportService = {
  getReport: (params: { date_start: string; date_end: string }) =>
