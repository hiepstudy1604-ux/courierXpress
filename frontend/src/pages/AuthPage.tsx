...
const CustomLogo = ({ size = 24, className = "" }: { size?: number, className?: string }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
    <path d="M12 2L3.5 6.5V17.5L12 22L20.5 17.5V6.5L12 2Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M12 22V12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M20.5 6.5L12 11.5L3.5 6.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M17 12.5L12 15.5L7 12.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.5" />
  </svg>
);
...
const AuthPage: React.FC<Props> = ({ onLogin, onBack, initialMode = 'login' }) => {
    ...
    try {
      if (isSignUp) {
        ...
        if (!passwordRules.isValid) {
          alert('Mật khẩu chưa đạt yêu cầu: tối thiểu 8 ký tự, có chữ hoa, chữ thường, số, ký tự đặc biệt; không có dấu cách và không có ký tự có dấu.');
          setIsLoading(false);
          return;
        }
        ...
        <div className="max-w-md w-full bg-white rounded-[40px] border border-slate-200 shadow-2xl p-10 lg:p-12 space-y-10">
          ...
          <div className="text-center space-y-4">
            <h1 className="text-lg font-medium text-slate-600 leading-relaxed">
              Enter the OTP that was just sent to your email<br />
          ...
          <div className="text-center space-y-8">
            <p className="text-sm font-bold text-slate-400">
              Didn't receive the OTP? <button className="text-[#f97316] font-black hover:underline transition-all">Resend</button>
            </p>
  ...
  return (
    <div className="h-screen bg-[#f3f4f6] flex overflow-hidden relative">
      <div className={`hidden lg:block w-1/2 p-12 lg:p-24 transition-colors duration-700 ${isStaffPortal ? 'bg-slate-900' : 'bg-[#f97316]'}`}>
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 lg:px-20 py-8 overflow-y-auto bg-white lg:bg-transparent">
        <div className="max-w-sm w-full mx-auto space-y-6">
          <div className="mt-4 mb-2">
            <button onClick={onBack} className="flex items-center text-slate-400 font-bold hover:text-[#f97316] transition-all group text-sm">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-500 font-medium">
              {isSignUp ? (
                <>Already have an account? <button onClick={() => { setIsSignUp(false); setIsStaffPortal(false); }} className="text-[#f97316] font-bold hover:underline">Log in now</button></>
              ) : (
                <>Don't have an account? <button onClick={() => { setIsSignUp(true); setIsStaffPortal(false); }} className="text-[#f97316] font-bold hover:underline">Sign up for free</button></>
              )}
            </p>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleAuth}>
            {isSignUp && (
              <div className="space-y-1.5 animate-in slide-in-from-top-2 duration-300">
                ...
                <div className="relative">
                  ...
                  <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full pl-11 pr-4 py-3.5 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500/40 shadow-sm transition-all font-medium text-sm text-slate-900" placeholder="Nguyễn Hoàng Hiệp" required />
                </div>
              </div>
            )}

            <div className="space-y-1.5">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">{isSignUp ? 'Phone or Email' : isStaffPortal ? 'Login ID' : 'Phone or Email'}</label>
              <div className="relative">
              ...
              {isSignUp && (
                <div className="pt-2 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-black uppercase tracking-widest text-slate-400">Password strength</span>
                    <span
                  ...
                  <div className="grid grid-cols-1 gap-1.5 text-xs font-bold">
                    <div className={passwordRules.minLength ? 'text-emerald-600' : 'text-slate-400'}>• Tối thiểu 8 ký tự</div>
                    <div className={passwordRules.hasUpper ? 'text-emerald-600' : 'text-slate-400'}>• Có chữ in hoa (A-Z)</div>
                    <div className={passwordRules.hasLower ? 'text-emerald-600' : 'text-slate-400'}>• Có chữ thường (a-z)</div>
                    <div className={passwordRules.hasNumber ? 'text-emerald-600' : 'text-slate-400'}>• Có số (0-9)</div>
                    <div className={passwordRules.hasSpecial ? 'text-emerald-600' : 'text-slate-400'}>• Có ký tự đặc biệt (vd: !@#)</div>
                    <div className={passwordRules.isAsciiPrintableNoSpace ? 'text-emerald-600' : 'text-rose-600'}>
            ...
            {isSignUp && (
              <div className="space-y-1.5 animate-in slide-in-from-top-2 duration-300">
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 transition-colors">
                    {showConfirmPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                  </button>
                </div>
                {confirmPassword.length > 0 && (
                  <p className={`text-xs font-bold ${confirmPassword === password ? 'text-emerald-600' : 'text-rose-600'}`}>
            ...
            <div className="pt-2 space-y-4">
              <button
                ...
                disabled={
                  isLoading ||
                  (isSignUp && (!passwordRules.isValid || confirmPassword !== password))
                }
              ...
              {!isStaffPortal && !isSignUp && (
                <button type="button" onClick={() => { setIsStaffPortal(true); setIsSignUp(false); }} className="w-full bg-white border-2 border-slate-100 text-slate-500 py-3.5 rounded-2xl font-black text-xs flex items-center justify-center gap-2 shadow-sm hover:bg-slate-50 hover:border-[#f97316]/30 hover:text-[#f97316] transition-all transform active:scale-[0.98]">
                  <ShieldCheck size={18} />
