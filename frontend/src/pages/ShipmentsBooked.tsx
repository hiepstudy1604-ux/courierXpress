...
> = ({
    ...
    const handleConfirm = async () => {
        try {
            ...
            // reflect in UI - this updates the rows state with new status
            ...
            alert("Branch assigned successfully. Moving to Assign Branch tab...");
        } catch (e: any) {
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                            <div>
                                ...
                                <p className="text-base font-black text-slate-900">
                                    {(shipment.fee / 25000).toFixed(2)} USD
                                </p>
                            </div>
                            <div>
                    ...
                    {selectedBranchId ? (
                        <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1">
                            ...
                            {branchVehicles.length === 0 && (
                                <p className="text-xs text-slate-400">No vehicles for this branch.</p>
                            )}
                        </div>
                    ) : (
                        <p className="text-xs text-slate-400 mt-2">
                            Please select a branch on the left to see available vehicles.
                        </p>
                    )}
                ...
                <div className="lg:col-span-12 space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-semibold text-slate-800">
                                        {formatPickupShift((pickupShift || null) as any)}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm">
                        ...
                        <p className="text-xs text-slate-500 mt-4">
                            Call Pending / Call Failed: <span className="font-semibold">Reschedule Pickup</span> is
                            active. Call Success: <span className="font-semibold">Schedule Pickup Success</span> is
                            active.
                        </p>
                    </div>
                </div>
...
> = ({ shipment, branches, onBack, onGoToPickup, onStayInSchedulePickup, onUpdateShipmentStatus }) => {
    const trackingId = shipment.trackingId || shipment.id;
    const shipmentId = shipment.id;
    const createdAt = shipment.bookingDate;
    const updatedAt = shipment.updatedAt || shipment.updated_at || null;
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-xs text-slate-600 mt-2">
                            ...
                            <div>
                                <p className="text-[11px] font-semibold text-slate-400 tracking-wide mb-0.5">Status</p>
                                <span
                                    className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold border ${statusPill}`}
                                >
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <p className="text-[11px] font-semibold text-slate-400 mb-0.5">Pickup Shift</p>
                                    <p className="text-xs font-semibold text-slate-800">
                                        {formatPickupShift((latestPickupShift || null) as any)}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm">
                        ...
                        <div className="space-y-4">
                            ...
                            {!isRescheduled ? (
                                ...
                                <p className="text-xs text-slate-500">
                                    Call Pending/Failed → <span className="font-semibold">Move To Issue</span> (Driver
                                    Status disabled). Call Success → Driver Status enabled (Ready → Set On The Way
                                    Pickup, Not Ready → Move To Issue).
                                </p>
                            )}
                        </div>
                    </div>
...
export default function ShipmentsBooked({ user, setView }: { user: UserType; setView?: (view: View) => void }) {
    ...
    useEffect(() => {
        (window as any).refreshShipmentsBooked = async (payload?: {
            trackingCode?: string;
            orderId?: string | null;
        }) => {
            await fetchData();
        ...
        // Listen for shipment:updated event from API calls
                    ...
                    <table className="w-full text-left border-collapse min-w-[1500px]">
                        <thead>
                            <tr className="bg-slate-50/80 border-b border-slate-100">
                                ...
                                {!isCustomer &&
                                    (activeTab === "BOOKED" ||
                                        activeTab === "ASSIGN_BRANCH" ||
                                        activeTab === "SCHEDULE_PICKUP") && (
                                        <th className="px-4 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">
                                            Action
                                        </th>
                                    )}
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr>
                                    <td
                                        colSpan={isCustomer ? 7 : activeTab === "BOOKED" ? 10 : 9}
