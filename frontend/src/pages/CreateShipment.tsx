...
const testSender: AddressInfo = {
  name: "Nguyễn Văn An",
  phone: "0987654321",
  address_detail: "Số 123, Đường ABC, Phường Dịch Vọng Hậu",
  province_code: "",
...
const testReceiver: AddressInfo = {
  name: "Trần Thị Bình",
  phone: "0912345678",
  address_detail: "456 Đường XYZ, Phường Bến Nghé",
  province_code: "",
...
const getTestFormData = (): FormData => ({
  ...
  payment_method: "CASH",
  note: "Giao hàng giờ hành chính. Vui lòng gọi trước.",
  idempotency_key: `idemp_test_${Date.now()}`,
});

export function CreateShipment({ user, setView }: Props) {
  ...
  // Whether the order has been confirmed successfully.
  ...
  const validateStep1 = (): boolean => {
    ...
    if (!formData.receiver.address_detail.trim()) newErrors.receiver_address_detail = "Detailed address is required";
    ...
    formData.products.forEach((product, index) => {
      ...
      if (formData.service_type === "STANDARD") {
        if (!product.length_cm || product.length_cm <= 0) newErrors[`product_length_${index}`] = "Length is required";
        if (!product.width_cm || product.width_cm <= 0) newErrors[`product_width_${index}`] = "Width is required";
        if (!product.height_cm || product.height_cm <= 0) newErrors[`product_height_${index}`] = "Height is required";
      ...
      if (product.declared_value <= 0) newErrors[`product_value_${index}`] = "Declared value must be greater than 0";
      if (product.declared_value > 10000000) {
        newErrors[`product_value_${index}`] = "Declared value exceeds the maximum insured amount (10,000,000 VND)";
      }
  ...
  const totalWeightG = useMemo(() => formData.products.reduce((sum, p) => sum + (p.weight_g || 0), 0), [formData.products]);
  const totalValue = useMemo(
    () => formData.products.reduce((sum, p) => sum + (p.declared_value || 0), 0),
    [formData.products]
  );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-xl bg-white rounded-[24px] shadow-2xl border border-slate-200 p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-xl font-black text-slate-900">Tạo đơn thành công</h2>
                <p className="text-sm text-slate-500 mt-1">Vui lòng xác nhận để chuyển sang tab Booked.</p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5 grid grid-cols-1 gap-3">
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Tracking ID</p>
                <p className="text-2xl font-black text-[#f97316] mt-1">{trackingCode || "—"}</p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-white border border-slate-200 rounded-2xl p-4">
                  <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Order ID</p>
                  <p className="text-sm font-bold text-slate-900 mt-1 break-all">{orderId || "—"}</p>
                </div>
                <div className="bg-white border border-slate-200 rounded-2xl p-4">
                  <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Phí dự kiến</p>
                  <p className="text-sm font-bold text-slate-900 mt-1">
                    {pricingBreakdown?.estimated_fee != null ? `${pricingBreakdown.estimated_fee.toLocaleString("en-US")} VND` : "—"}
                  </p>
                </div>
              </div>
            </div>
          ...
          {timelineSteps.map((step, index) => {
            ...
            return (
              <React.Fragment key={step.number}>
                <div className="flex flex-col items-center">
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center font-black text-sm transition-all ${
                      isActive ? "bg-[#f97316] text-white shadow-lg" : "bg-slate-200 text-slate-400"
                ...
                {index < timelineSteps.length - 1 && <div className="w-16 h-0.5 bg-slate-200 rounded" />}
              </React.Fragment>
            );
          })}
        </div>
          ...
          <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  />
                  {errors.sender_address_detail && <p className="text-xs text-red-600">{errors.sender_address_detail}</p>}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  {errors.sender_province && <p className="text-xs text-red-600">{errors.sender_province}</p>}
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData((prev) => ({ ...prev, receiver: { ...prev.receiver, name: e.target.value } }))}
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData((prev) => ({ ...prev, receiver: { ...prev.receiver, phone: e.target.value } }))}
                    placeholder="Phone number"
                  />
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="Detailed address"
                  />
                  {errors.receiver_address_detail && <p className="text-xs text-red-600">{errors.receiver_address_detail}</p>}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  {errors.receiver_province && <p className="text-xs text-red-600">{errors.receiver_province}</p>}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  {errors.receiver_ward && <p className="text-xs text-red-600">{errors.receiver_ward}</p>}
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => setFormData((prev) => ({ ...prev, pickup_slot: e.target.value as PickupSlot }))}
                >
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({ ...prev, inspection_policy: e.target.value as InspectionPolicy }))
                  }
                >
            ...
            <div className="mt-8">
              ...
              <p className="text-xs text-slate-500 mb-3">
                Total: {(totalWeightG / 1000).toFixed(2)} kg
              </p>
                  ...
                  <div key={product.id} className="border border-slate-200 rounded-2xl p-4">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                      <div className="space-y-1">
                        ...
                        />
                        {errors[`product_name_${index}`] && <p className="text-xs text-red-600">{errors[`product_name_${index}`]}</p>}
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) => updateProduct(product.id, { weight_g: Number(e.target.value) })}
                        ...
                        {errors[`product_weight_${index}`] && <p className="text-xs text-red-600">{errors[`product_weight_${index}`]}</p>}
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) => updateProduct(product.id, { declared_value: Number(e.target.value) })}
                          placeholder="Declared value (VND)"
                          min={0}
                        />
                        {errors[`product_value_${index}`] && (
                          <p className="text-xs text-red-600">{errors[`product_value_${index}`]}</p>
                        )}
                      </div>
                        ...
                        <>
                          <div className="space-y-1">
                            ...
                            <input
                              ...
                              onChange={(e) => updateProduct(product.id, { length_cm: Number(e.target.value) })}
                              placeholder="Length (cm)"
                              min={0}
                            />
                            {errors[`product_length_${index}`] && (
                              <p className="text-xs text-red-600">{errors[`product_length_${index}`]}</p>
                            )}
                          </div>
                          ...
                          <div className="space-y-1">
                            ...
                            <input
                              ...
                              onChange={(e) => updateProduct(product.id, { width_cm: Number(e.target.value) })}
                              placeholder="Width (cm)"
                              min={0}
                            />
                            {errors[`product_width_${index}`] && (
                              <p className="text-xs text-red-600">{errors[`product_width_${index}`]}</p>
                            )}
                          </div>
                          ...
                          <div className="space-y-1">
                            ...
                            <input
                              ...
                              onChange={(e) => updateProduct(product.id, { height_cm: Number(e.target.value) })}
                              placeholder="Height (cm)"
                              min={0}
                            />
                            {errors[`product_height_${index}`] && (
                              <p className="text-xs text-red-600">{errors[`product_height_${index}`]}</p>
                            )}
                          </div>
                        </>
                      )}
                      ...
                      {formData.service_type === "EXPRESS" && (
                        <div className="space-y-1">
                          <select
                            ...
                            onChange={(e) => updateProduct(product.id, { express_size: e.target.value as ExpressSize })}
                          >
                          ...
                          {errors[`product_size_${index}`] && (
                            <p className="text-xs text-red-600">{errors[`product_size_${index}`]}</p>
                          )}
                        </div>
                      )}
                      ...
                      <div className="space-y-1">
                        ...
                        {errors[`product_category_${index}`] && (
                          <p className="text-xs text-red-600">{errors[`product_category_${index}`]}</p>
                        )}
                      </div>
                      ...
                      <div className="lg:col-span-3">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs text-slate-500">Max 4 images per item</span>
                        </div>
                        {errors[`product_images_${index}`] && (
                          <p className="text-xs text-red-600 mt-2">{errors[`product_images_${index}`]}</p>
                        )}
                          ...
                          <div className="mt-3 flex flex-wrap gap-3">
                            {product.imagePreviews.map((src, imgIndex) => (
                              <div key={src} className="relative">
                                <img src={src} className="w-20 h-20 object-cover rounded-xl border" />
                                <button
                                  type="button"
          ...
          <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                ...
                <p className="text-2xl font-black text-slate-900">{pricingBreakdown.estimated_fee.toLocaleString("en-US")} VND</p>
              </div>
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                <p className="text-xs text-slate-500 font-bold">Total weight</p>
                <p className="text-2xl font-black text-slate-900">{(pricingBreakdown.chargeable_weight ?? totalWeightG / 1000).toFixed(2)} kg</p>
              </div>
            </div>
